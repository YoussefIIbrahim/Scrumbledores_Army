...
var config = require('./serverConfigs');// Create custom secret to use with JWT

module.exports = function(app, passport) {
    // Start Passport Configuration Settings
    app.use(passport.initialize());
    app.use(passport.session());
    app.use(session({ secret: 'scrumbledore', resave: false, saveUninitialized: true, cookie: { secure: false } }));
    ...
    passport.serializeUser(function(user, done) {
        ...
        if (user.active) {
            ...
            if (user.error) {
                ...
                token = jwt.sign({ username: user.username, email: user.email }, config.JWTKey, { expiresIn: '4h' }); // If account active, give user token
            }
        } else {
